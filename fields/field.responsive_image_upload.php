<?php
	...
	class fieldResponsive_image_upload extends fieldUpload
	{
		...
		public function __construct(){
			...
			$this->_name = __( 'Responsive Image Upload' );
		}
		 ...
		 */
		public static function resize($file, $width, $height, $mimetype, $saveas = null){
			...
			if( $jit_status[0] === EXTENSION_ENABLED ){
				...
				if (!is_null($saveas)) {
					$image->save( $saveas, 85, null, $mimetype );
				} else {
		...
		public function appendFormattedElement(&$wrapper, $data){
			...
			if(!empty($field)) {
				$emt = new XMLElement('responsive-sizes');
				$responsive_sizes_array = explode(',', $this->get('responsive_sizes'));
				if (!empty($responsive_sizes_array)) {
					$image_path_parts = pathinfo($this->getFilePath($data['file']));
		...
		public function displaySettingsPanel(XMLElement &$wrapper, $errors = null){
			...
			$this->_addDimensionInput(
				$div,
				__( 'Responsive widths (comma-seperated in px)' ),
				'responsive_sizes',
				__( 'If empty or 0, no responsive versions of this images will be saved.' )
			);
		...
		public function commit(){
			...
			$settings['responsive_sizes']  = $this->get( 'responsive_sizes' );
		...
		public function processRawFieldData($data, &$status, &$message, $simulate = false, $entry_id = null){
			...
			if( is_string( $data ) ){
				...
				if( $this->isResizeActive() && $result['mimetype'] !== 'application/octet-stream' ){
					...
					if( (!empty($max_width) && ($max_width > 0)) || (!empty($max_height) && ($max_height > 0)) ){
						...
						$file = WORKSPACE.$destination.$result['file'];
						if( is_file( $file) ){
			...
			$responsive_sizes_array = explode(',', $this->get('responsive_sizes'));
			if (!empty($responsive_sizes_array) && $file) {
